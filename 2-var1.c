...
Graph initGraph(int V) { 
	...
	for (i = 0; i < V; i++) {
		...
		elem += 5; // 1 comparatie, 1 incrementare, 
				   // 1 accesare tip '->', 1 indexare, 1 atribuire
	}
 ...
 */
// Sortarea
void sort(Pair* s, int n, int* poz) {
	int done = 0, i;
	Pair aux;
	while(!done) {
		done = 1;
...
// Extragerea primului element (minimul)
int extractMin(Pair* s, int *n, int* poz) {
	int r = s[0].v, i;
	for(i = 0; i < (*n) - 1; i++) {
		poz[s[i + 1].v]--;
		s[i] = s[i + 1];
